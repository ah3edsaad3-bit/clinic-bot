...
def add_user_message(user_id, text):
    ...
    # إنشاء الجلسة أو إعادة تهيئتها
    if (
        user_id not in SESSIONS
        or (now - SESSIONS[user_id].get("last_message_time", 0) > MEMORY_TIMEOUT)
    ):
        SESSIONS[user_id] = {
            "history": [],
            "name": "",
            ...
            "followup_sent": False,
            "greeted": False      # 👈 إضافة الفلاغ
        }

    st = SESSIONS[user_id]

    # 👋 إرسال الإيموجي أول مرة فقط
    if not st["greeted"]:
...
# =======================================================
# 🤖 GPT Handler — ANSWER ONLY LAST MESSAGE
# =======================================================
...
وضيفتك ترد على الرسائل باللهجة العراقية العامية ، وبدون مبالغة وتجاوب على جميع استفساراتهم بطريقة تطمن المراجع ويكون جواب وافي عن كل شي يخص طب الاسنان ، 
...
3. عدم تكرار الترحيب: الترحيب مرة واحدة فقط، بعدها ادخل بالجواب فوراً.
4. الاختصار: جوابك سطرين فقط بدون اي مبالغة
...
مثال للردود الصح:
 • حاضر حبي ❤️
 • تمام، هسه احسبلك السعر
 • يعتمد حسب عدد الأسنان
 • عاشت إيدك، وضحت الفكرة
 • راح أرتّبلك كلشي هسة
...
14: الايماكس سعرة 150 الف للسن الواحد ( قشور الايماكس )
...
20: سعر الفك المتحرك او التركيب المتحرك يكون 40 الف للسن الواحد"""
